...
@interface DFQRCodeScanningView ()
...
@implementation DFQRCodeScanningView
/** 二维码冲击波动画时间 */
static CGFloat const DFQRCodeScanningLineAnimation = 0.05;
...
#pragma mark - - - 扫描边角imageView的创建
    ...
    UIImage *left_image = [UIImage imageNamed:@"CodeLeftTop"];
    ...
    // 右上侧的image
    UIImage *right_image = [UIImage imageNamed:@"CodeRightTop"];
    ...
    // 左下侧的image
    UIImage *left_image_down = [UIImage imageNamed:@"CodeLeftBottom"];
    ...
    // 右下侧的image
    UIImage *right_image_down = [UIImage imageNamed:@"CodeRightBottom"];
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        _scanningline.image = [UIImage imageNamed:@"CodeScanningLine"];
...
- (void)addTimer {
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:DFQRCodeScanningLineAnimation target:self selector:@selector(timeAction) userInfo:nil repeats:YES];
...
- (void)timeAction {
    ...
    if (flag) {
        ...
        flag = NO;
        [UIView animateWithDuration:DFQRCodeScanningLineAnimation animations:^{
            frame.origin.y += 5;
